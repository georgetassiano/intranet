      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Lista de processos cadastrados</h3>
            <div class="box-tools pull-right">
              <!-- Buttons, labels, and many other things can be placed here! -->
              <!-- Here is a label for example -->
              <router-link to="/processos/novoprocesso" tag="button" class="btn btn-success"><i class="fa fa-plus"> Novo Processo</router-link>
              <button @click="getProcessos" class="btn btn-info"><i class="fa fa-refresh"> Atualizar</button>
            </div><!-- /.box-tools -->
          </div>
          <!-- /.box-header -->
          <div class="box-body">
              ...
              <div class="row">
                <div class="col-sm-12">
                  <table aria-describedby="example1_info" role="grid" id="example1" class="table table-bordered table-striped dataTable table-responsive">
                    ...
                    <tbody>
                      <tr role="row" v-for="(processo,index) in processos" :class="paridade(index)">
                        <td class="sorting_1">{{processo.nome|capitalize}} {{paridade(index)}}</td>
                        <td>
                          <router-link :to="{path:'/processos/verprocesso', query:{index:index}}" tag="button"  class="btn btn-primary btn-sm"><i class="fa fa-search"> Ver</router-link>
                          <router-link :to="{path:'/processos/editarprocesso', query:{index:index}}" tag="button"  class="btn btn-warning btn-sm"><i class="fa fa-pencil"> Alterar</router-link>
                          <router-link to="/processos"><button @click="deletar(processo.id)" class="btn btn-danger btn-sm"><i class="fa fa-remove"> Remover</button></router-link>
                        </td>
                      </tr>
                    </tbody>
                    <tfoot>
...
<script>
  ...
  export default {
    ...
    methods: {
      deletar (id) {
        this.$http.delete('http://localhost:8000/dados/processos/' + id).then((response) => {
