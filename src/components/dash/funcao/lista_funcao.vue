  ...
  <div>
        ...
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">Lista de cargos cadastrados</h3>
						<div class="box-tools pull-right">
							<!-- Buttons, labels, and many other things can be placed here! -->
							<!-- Here is a label for example -->
							<router-link to="/funcoes/novafuncao" tag="button" class="btn btn-success"><i class="fa fa-plus"> Novo Cargo</router-link>
							<button @click="getFuncoes" class="btn btn-info"><i class="fa fa-refresh"> Atualizar</button>
						</div><!-- /.box-tools -->
          </div>
          <!-- /.box-header -->
          <div class="box-body">
              ...
              <div class="row">
                <div class="col-sm-12">
                  <table aria-describedby="example1_info" role="grid" id="example1" class="table table-bordered table-striped dataTable table-responsive">
                    ...
                    <tbody>
											<tr role="row" v-for="(funcao,index) in funcoes" :class="paridade(index)">
                        <td class="sorting_1">{{funcao.nome|capitalize}}</td>
									  		<td>
									  			<router-link :to="{path:'/funcoes/verfuncao', query:{index:index}}" tag="button"  class="btn btn-primary btn-sm"><i class="fa fa-search"> Ver</router-link>
									  			<router-link :to="{path:'/funcoes/editarfuncao', query:{index:index}}" tag="button"  class="btn btn-warning btn-sm"><i class="fa fa-pencil"> Alterar</router-link>
									  			<router-link to="/funcoes"><button @click="deletar(funcao.id)" class="btn btn-danger btn-sm"><i class="fa fa-remove"> Remover</button></router-link>
								    		</td>
                      </tr>
                    </tbody>
                    <tfoot>
...
<script>
  ...
  export default {
    ...
    methods: {
      deletar (id) {
        this.$http.delete('http://localhost:8000/dados/funcoes/' + id).then((response) => {
