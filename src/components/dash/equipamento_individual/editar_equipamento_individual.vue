          ...
          <div class="box box-info">
            ...
            <div class="box-header with-border">
              ...
              <div class="box-tools pull-right">
                ...
                <router-link to="/equipamentosindividuais" tag="button" class="btn btn-danger btn-sm"><i class="fa fa-remove"> Fechar</router-link>
              </div><!-- /.box-tools -->
            </div>
            <div class="box-body">
  ...
  export default {
    ...
    computed: {
      equipamentoindividual () {
        return this.$store.state.equipamentosindividuais[this.$route.query.index]
      }
    },
    watch: {
      equipamentoindividual () {
    ...
    mounted () {
      this.nome = this.$store.state.equipamentosindividuais[this.$route.query.index].nome
    },
    methods: {
      alterar () {
        this.$http.put('http://localhost:8000/dados/equipamentosindividuais/' + this.equipamentoindividual.id, {
          'Nome': this.nome
        }).then((response) => {
          console.log('sucess ' + response)
          this.$store.commit('ATUALIZAR_EQUIPAMENTOINDIVIDUAL', {
