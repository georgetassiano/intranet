...
<script>
  export default {
    name: 'editarequipamentosindividuais',
    data () {
      return {
        checkedEquipamentos: [],
        analiseriscoidCopia: '',
    ...
    methods: {
      enviar () {
        this.$store.state.analisesrisco[this.$route.query.index].equipamentosIndividuais = []
        this.$http.delete('http://endicon.esy.es/dados/equipamentosindividuaisanaliserisco/' + this.analiseriscoid).then((response) => {
